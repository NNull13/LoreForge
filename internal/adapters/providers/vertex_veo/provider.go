...
func (p Provider) GenerateVideo(ctx context.Context, input contracts.VideoRequest) (contracts.VideoResponse, error) {
	...
	mediaURL, err := files.GCSMediaURLWithBase(outputURI, firstNonEmpty(optionString(p.Config.Options, "gcs_base_url", ""), "https://storage.googleapis.com"))
	if err != nil {
		return contracts.VideoResponse{}, err
	}
	path, _, err := files.DownloadToTemp(ctx, client.HTTP, mediaURL, "vertex-veo", map[string]string{
		"Authorization": "Bearer " + token,
	})
	if err != nil {
		return contracts.VideoResponse{}, err
