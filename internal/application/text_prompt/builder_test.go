...
func TestBuildTweetThreadIncludesSchemaAndReferences(t *testing.T) {
	...
	}, episode.OutputTypeTweetThread, text_settings.ResolvedTextSettings{
...
func TestBuildShortStoryIncludesPresentationPolicyWhenDisabled(t *testing.T) {
	...
	}, episode.OutputTypeShortStory, text_settings.ResolvedTextSettings{
...
func TestBuildSystemPromptCoversFormats(t *testing.T) {
	...
	for _, tt := range cases {
		if got := buildSystemPrompt(tt.format, text_settings.ResolvedTextSettings{MinParts: 2, MaxParts: 4}); !strings.Contains(got, tt.want) {
...
func TestPromptHelperFormatting(t *testing.T) {
	...
	shortSchema := schemaFor(episode.OutputTypeTweetShort, text_settings.ResolvedTextSettings{})
	if shortSchema["type"] != "object" {
		t.Fatalf("unexpected short schema: %#v", shortSchema)
	}
	defaultSchema := schemaFor(episode.OutputTypeShortStory, text_settings.ResolvedTextSettings{})
