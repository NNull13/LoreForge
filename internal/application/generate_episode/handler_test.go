...
func TestHandleGeneratesAndPublishesEpisode(t *testing.T) {
	...
	handler := Handler{
		...
		EpisodeRepo:  repo,
		Scheduler:    scheduler.Service{StateRepo: schedulerRepo},
...
func TestHandleRetriesInvalidOutput(t *testing.T) {
	...
	handler := Handler{
		...
		Scheduler:    scheduler.Service{StateRepo: &fakeSchedulerStateRepo{}},
		GeneratorRegistry: fakeGeneratorRegistry{
			items: []ports.RegisteredGenerator{
				{
...
func TestHandleReturnsNoGeneratorsDue(t *testing.T) {
	...
	handler := Handler{
		UniverseRepo: fakeUniverseRepo{universe: testUniverse()},
		EpisodeRepo:  &fakeEpisodeRepo{},
		Scheduler:    scheduler.Service{StateRepo: &fakeSchedulerStateRepo{states: map[string]scheduling.State{"short-story-artist": {NextRunAt: now.Add(time.Hour)}}}},
		GeneratorRegistry: fakeGeneratorRegistry{
			items: []ports.RegisteredGenerator{
				{
...
func TestPublishFailureErrorAndPublishedChannels(t *testing.T) {
	...
	results := map[string]any{
		...
		"twitter":    publication.Result{Channel: "twitter", Success: false, Message: "timeout"},
	}
	...
	if err := publishFailureError(map[string]any{"twitter": publication.Result{Channel: "twitter", Success: false, Message: "timeout"}}, []publication.Target{{Channel: publication.ChannelTwitter}}); !errors.Is(err, episode.ErrPublishFailed) {
...
func TestSchedulerServiceUsesSavedStateAndSchedulerFallback(t *testing.T) {
	t.Parallel()
	...
	service := scheduler.Service{StateRepo: &fakeSchedulerStateRepo{
	...
	if err != nil {
		t.Fatalf("Scheduler.NextRun with state returned error: %v", err)
	}
	if want := now.Add(30 * time.Minute); !next.Equal(want) {
		t.Fatalf("next run = %s, want %s", next, want)
	}

	next, err = scheduler.Service{StateRepo: &fakeSchedulerStateRepo{}}.NextRun(context.Background(), def, now)
	if err != nil {
		t.Fatalf("Scheduler.NextRun fallback returned error: %v", err)
	}
...
func TestNextDueGeneratorAndPublishHelpers(t *testing.T) {
	...
	if result, ok := results["filesystem"].(publication.Result); !ok || result.Message != "channel not configured" {
...
func TestTemplateUniverseDataAndSnapshotHelpers(t *testing.T) {
	...
	if snapshot["id"] != "ash-chorister" || snapshot["non_diegetic"] != true {
